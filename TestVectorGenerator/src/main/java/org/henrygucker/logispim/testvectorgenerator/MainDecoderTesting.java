...
public class MainDecoderTesting {
    ...
    private static final String COMPONENT_FLAG = "--main-decoder";
    ...
    public MainDecoderTesting(String[] args) throws IllegalArgumentException, IOException {
        if (args.length == 0)
            throw new IllegalArgumentException("Second argument expected, but not provided.\n" +
                    ...
                    "java -jar "+ Main.PROGRAM_JAR_NAME + this.COMPONENT_FLAG + "--validate-markdown-tables <path to file for analysis>");
        ...
        if (args[0].equals("--create-test-vector")) {
            ...
            System.out.println("Output successfully written to \"" + outputFilepath.toAbsolutePath().toString() + "\".");
            ...
            assert(containsMarkdownTables(outputFilepath));
        } else {
    ...
    private static void writeOutputFile(Path filepath) throws IOException {
