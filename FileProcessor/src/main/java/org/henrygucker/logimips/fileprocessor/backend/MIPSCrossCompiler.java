...
import org.henrygucker.logimips.fileprocessor.backend.exceptions.CompilationException;
    ...
    private void preprocessor(Path destination) throws IOException {
        ...
        try (
            ...
            while ((inn = bufferedReader.readLine()) != null) {
                for (String split : Arrays.stream(inn.split("#"))
                        .limit(1)
                    ...
                    if (!isKernelText && split.trim().equals("syscall"))
                        bufferedWriter.write("\tmove $k0, $ra\n");
