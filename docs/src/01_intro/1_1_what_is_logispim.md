...
This is accomplished through the following process in which source code is compiled into files that can be easily
imported into Logisim-Evolution, where the program is executed.
...
Some information about the Logisim Processor and File Processor components of this workflow is provided below.
...
Since each stage has its own---isolated---purpose, this allows for multiple instructions to be executed at the same time.
...
2. The instruction is decoded and contents of relevant general-purpose registers are read
