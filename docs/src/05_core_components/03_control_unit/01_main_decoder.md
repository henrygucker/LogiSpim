...
### WriteRegSrc
Relevant Pipeline Stage(s): `(3) Execute`<br>
Bit Width: `2`<br>
Category: `Multiplexer Control`

This signal decides which portion of an instruction specifies the destination register.
The values of `WriteRegSrc` are mapped to values of `WriteReg` (or the register that would be written to)
by the following table:

|  #  | Purpose                 |
|:---:|:------------------------|
| `0` | Sets `WriteReg` to `rt` |
| `1` | Sets `WriteReg` to `rd` |

The function of this multiplexer is pictured below:

![WriteRegSrc controlling WriteReg value](assets/write_reg_src.png)
...
### Outputs by Op Code
The following table lists all defined Main Decoder output values for every given Op code.

| #  |    OP    |             Instruction             |  1  |  2   |   3    |  4   |   5    |  6  |  7   |  8  |
|:--:|:--------:|:-----------------------------------:|:---:|:----:|:------:|:----:|:------:|:---:|:----:|:---:|
| 00 | `000000` | `R-Type`[*](#overrides-for-r-types) | `1` | `00` | `0000` | `00` | `1000` | `0` | `00` | `1` |
| 01 | `000001` |                `N/A`                |     |      |        |      |        |     |      |     |
...
> [!NOTE]
> Since these overrides only occur for R-Type instructions, the `Funct` value is only able to
> affect the output values when the OP code is `0`.